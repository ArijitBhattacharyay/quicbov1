...
_ready = False
_platform_prewarm_tasks: dict[str, asyncio.Task] = {}  # Tracks per-platform location setting tasks


PLATFORM_IDS = ["blinkit", "zepto", "bigbasket"]
...
async def _launch_browser(pid: str):
    ...
    try:
        ...
        browser = await _playwright.chromium.launch(
            headless=True,  # Run in background without popping windows
            args=LAUNCH_ARGS,
            slow_mo=0,
        )
...
async def _clear_overlays(page: Page) -> None:
    """Dismiss full-page overlays/modals that block interaction in headless mode."""
    try:
        # Try Esc key first
        await page.keyboard.press("Escape")
        # Direct JS to hide common high z-index blockers if they are visible
        await page.evaluate("""() => {
            const blockers = document.querySelectorAll('div[class*="fixed"], div[style*="z-index: 99999"], div[class*="modal"]');
            for (let el of blockers) {
                const style = window.getComputedStyle(el);
                if (style.display !== 'none' && parseInt(style.zIndex) > 1000) {
                   // Only hide if it's broad/centered (likely a modal)
...
class BlinkitScraper:
    ...
    async def search(self, product: str) -> list[ProductResult]:
                ...
                try:
                    ...
                    for item in items:
                        captured_results.append(ProductResult(
                            name=item.get("name", "—"),
                            weight=item.get("unit", "—") or item.get("quantity", "—"),
                            price=float(item.get("price", 0)) / 100 if "price" in item else 0,
                            original_price=str(item.get("mr_price", "")) if "mr_price" in item else "",
        ...
        await self.page.wait_for_timeout(3000)
        if captured_results:
            print(f"[blinkit] captured {len(captured_results)} products via API")
...
class ZeptoScraper:
    ...
    async def search(self, product: str) -> list[ProductResult]:
                ...
                try:
                    ...
                    for p in data.get("products", []):
                        captured_results.append(ProductResult(
                            ...
                            price=float(p.get("selling_price", 0)) / 100,
                            original_price=str(p.get("mrp", 0) / 100),
                            discount=p.get("discount_formatted", ""),
                            available=p.get("is_out_of_stock") is False,
                            platform="zepto",
                            image=f"https://cdn.zepto.co.in/browser/prod/{p.get('image_url', '')}",
                            delivery_time=15
                        ))
                except Exception: pass
        ...
        if not typed:
            ...
            await self.page.keyboard.press("Enter"); await self.page.wait_for_timeout(1000)

        await self.page.wait_for_timeout(3000)
        if captured_results:
            print(f"[zepto] captured {len(captured_results)} products via API")
            return captured_results
...
async def prewarm_location(pincode: str) -> dict:
    ...
    async def _do_prewarm():
        ...
        async def _set_one_and_track(pid: str):
            # If a task for this platform is already running, cancel it
            if pid in _platform_prewarm_tasks and not _platform_prewarm_tasks[pid].done():
                _platform_prewarm_tasks[pid].cancel()
            
            async def _task_logic():
                page = _get_page(pid)
                if page is None: return pid, "no_page"
                try:
                    scraper = SCRAPERS[pid](page)
                    await scraper.set_location(pincode)
                    return pid, "ok"
            ...
            task = asyncio.create_task(_task_logic())
            _platform_prewarm_tasks[pid] = task
            return await task

        tasks = [_set_one_and_track(pid) for pid in PLATFORM_IDS]
...
async def run_all_parallel(pincode: str, product: str) -> list[TestReport]:
    ...
    if not skip_location:
         # NEW: On mismatch, we must clear/trigger a new prewarm for relevant platforms
         # For simplicity, we trigger a prewarm_location call, but don't await the whole thing
         # as we'll await platform tasks individually below.
         asyncio.create_task(prewarm_location(pincode))
         print(f"[live_agent] pincode mismatch ({_warmed_pincode!r} != {pincode!r}) — triggered inline prewarm")
    ...
    async def _run_one(pid: str) -> TestReport:
        ...
        if page is None:
            ...
            return report
        try:
            # Pipelining: Await only THIS platform's pre-warm task if it exists
            if pid in _platform_prewarm_tasks:
                task = _platform_prewarm_tasks[pid]
                if not task.done():
                    print(f"[live_agent] {pid} pipelining: awaiting its specific prewarm task...")
                    await task
            
            scraper = SCRAPERS[pid](page)
            # If we didn't skip location (mismatch), but for some reason no task was created, 
            # set it manually to be safe. But with the inline prewarm trigger above, 
            # there should always be a task.
            if not skip_location and (pid not in _platform_prewarm_tasks or _platform_prewarm_tasks[pid].exception()):
                await scraper.set_location(pincode)
